...
n_alojamientos <- local_activo %>% group_by(host_id,host_name) %>% summarize(alojamientos = n() ) %>% arrange(desc(alojamientos))

n_alojamientos <- data.frame(n_alojamientos) # crea data frame

kable(n_alojamientos[1:30,],caption = "Los 30 anfitriones con más alojamientos")
...
ggplot(data=n_alojamientos, aes(alojamientos)) + 
  geom_histogram(breaks=seq(0, 200, by = 1), 
                 ...
                 fill="#999999", 
                 alpha = .2) + 
  labs(title="Histograma de alojamientos por anfitrión") +
  labs(x="número de alojamientos", y="número de alojamientos por anfitrión") + 
  xlim(c(0,200)) 
...
```{r table_anfitriones_distribucion, echo = FALSE, warning=FALSE,, eval=TRUE}
ggplot(data=naccommodates, aes(accommodates_sum)) + 
  geom_histogram(breaks=seq(0, 1100, by = 1), 
                 col="#999999", 
                 fill="#999999", 
                 alpha = .2) + 
  labs(title="Histograma de plazas por anfitrión") +
  labs(x="número de plazas", y="número de plazas por anfitrión") + 
  xlim(c(0,1100)) 
``` 
```{r final_output_distribucion_1, echo = FALSE, warning=FALSE,, eval=TRUE}
distribucion <- data.frame(matrix(0, ncol = 4, nrow = 3))
names(distribucion) <- c("n1","n2","n3.4","mas_de_5")
...
distribucion$n1[3] <- sum(naccommodates[naccommodates$host_id %in% n_alojamientos[n_alojamientos$alojamientos == 1,]$host_id,]$accommodates_sum)
distribucion$n2[3] <- sum(naccommodates[naccommodates$host_id %in% n_alojamientos[n_alojamientos$alojamientos == 2,]$host_id,]$accommodates_sum)
distribucion$n3.4[3] <- sum(naccommodates[naccommodates$host_id %in% n_alojamientos[(n_alojamientos$alojamientos == 3 | n_alojamientos$alojamientos == 4),]$host_id,]$accommodates_sum)
distribucion$mas_de_5[3] <- sum(naccommodates[naccommodates$host_id %in% n_alojamientos[n_alojamientos$alojamientos > 4,]$host_id,]$accommodates_sum)
