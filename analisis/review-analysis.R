...
# Create date field
reviews$datex <- as.Date( paste(as.numeric(strapplyc( as.character(reviews$date), ".*[0-9]*-[0-9]*-([0-9]*)", simplify = TRUE)),"/",
                                as.numeric(strapplyc( as.character(reviews$date), "[0-9]*-([0-9]*)-[0-9]*", simplify = TRUE)),"/",
                                as.numeric(strapplyc( as.character(reviews$date), "([0-9]*).*", simplify = TRUE)),sep = "" ), "%d/%m/%Y")

# Sve data if needed
save(reviews,file="data/original/airbnb/180818/temp/reviews_summary_barcelona_insideairbnb.Rda")
...
png(filename="images/airbnb/reviews/airbnb-reviews-puntos-barcelona-2010-2018.png",width = 900,height = 600)
ggplot(reviews[reviews$listing_id %in% unique(listings$id),], aes(datex,factor(listing_id))) +
  ...
  geom_point(aes(),position = "jitter", alpha=0.01,size=0.005) + # for big plot geom_point(aes(),position = "jitter", alpha=0.5,size=0.1) 
labs(title = "Reviews por fecha y número de id de alojamiento Airbnb. Barcelona 2010-2018 (agosto)",
     ...
     x = "Fecha de review",
     y = "id de alojamiento",
     caption = "Efecto Airbnb. lab.montera34.com. Datos: InsideAirbnb de agosto 2018") +
...
# Donostia
png(filename="images/airbnb/reviews/airbnb-barcelonareviews-mes-2010-2018.png",width = 900,height = 600)
...
# Faceted
png(filename="images/airbnb/reviews/airbnb-barcelona-reviews-mes-2010-2018_faceted.png",width = 900,height = 600)
...
listings_top <- listings[listings$host_id==1391607 |listings$host_id==396363 |listings$host_id== 158023606 |listings$host_id== 1503151,]$id

# Use this if do not know which listings to select
# listings_top <- listings[1:100,]
png(filename="images/airbnb/reviews/airbnb-reviews-barcelona-mes-top4-marcado-2010-2018_g2.png",width = 2700,height = 1800)
ggplot() +
  ...
  geom_point(data=reviews[reviews$listing_id %in% listings_top,],aes(datex,factor(listing_id)),
             alpha=1,size=2,color="#FF0000") +
  labs(title = "Reviews de Airbnb por mes en Barcelona 2010-2018 (agosto)",
     subtitle = "Cada línea es un listing. En rojo los top 4 anfitriones que gestionan xxx alojamientos.",
     x = "Año",
     y = "id de alojamiento",
     caption = "Efecto Airbnb. lab.montera34.com Data: InsideAirbnb") +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") 
