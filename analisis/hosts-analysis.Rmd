---
title: "Efecto Airbnb en Madrid: hosts y licencias"
date: "`r Sys.Date()`"
...
local_activo <- read.delim("../data/original/airbnb/190308/data/listings.csv.gz",sep = ",")
# local_activo <- read.delim("../../madrid/data/original/airbnb/2019-03-08/data/listings.csv.gz",sep = ",")
...
# Madrid
# barrios <- readOGR("../../madrid/data/output/contornos/barrios-madrid.geojson")
# municipios <- barrios 
...
labs(title=paste("Anuncios en ", local_activo_name, " por tipo de alojamiento y host", sep = ""),
     subtitle = "Datos: InsideAirbnb 2019-03-08",
     caption = caption_1,
...
geom_polygon(data = barrios,
             ...
             color = "grey", fill="white", size = 0.3) +
geom_point(data= local_activo,
         aes(x=longitude, y=latitude, color=host.type ),alpha=0.2,size = 0.7) + 
coord_quickmap(xlim=coord_x, ylim=coord_y)  +
theme_nothing(legend = TRUE) +
theme_minimal(base_family = "Roboto Condensed", base_size = 12) +
theme(
...
labs(title=paste("Anuncios por tipo de alojamiento y número de anuncios gestiona host", sep = ""),
     subtitle = paste("Airbnb en ",local_activo_name,". Datos: InsideAirbnb 2019-03-08",sep=""),
     caption = caption_1,
     color="Tipo alojamiento")  +
guides(colour = guide_legend(override.aes = list(size=3))) +
...
#   print(paste("../data/original/airbnb/190308/images/top-",i,".jpg",sep=""))
#   download.file(as.character(n_alojamientos.url$host_picture_url[i]),
#             destfile=paste("../data/original/airbnb/190308/images/top-",i,"-",substr(n_alojamientos.url$host_name[i], start=1, stop=3),".jpg",sep=""),
#             method = "wget")
#     download.file(as.character(n_alojamientos.url$host_picture_url[i]),
#             destfile=paste("../data/original/airbnb/190308/images/tophost-",i,".jpg",sep=""),
#             method = "wget")
# }
...
```{r table_propietarios.2, echo = FALSE, warning=FALSE, eval=TRUE, fig.width=5, fig.height=8}
local_activo %>% group_by(host_id, host_name, room_type) %>% summarize(alojamientos = n(),plazas = sum(accommodates) ) %>% arrange(desc(alojamientos)) %>% filter(alojamientos > 25) %>%
ggplot(aes(x = reorder(host_name,alojamientos), y = alojamientos,fill=room_type)) +
  ...
  labs(title = "Anfitriones en Airbnbcon más de 25 anuncios",
...
# 
#   png(filename=paste("images/airbnb/hosts/201903_top",j,"-hosts-bcn.png", sep = ""), width = 300, height = 190 )
#     p1 <- ggplot() +
#     geom_polygon(data = municipios,
#            aes(x = long, y = lat, group = group),
#            color = "grey", fill="white", size = 0.1) +
...
# montage top* -geometry +4+3 top-hosts-bcn-092018-airbnb.png
```
...
ggplot(dist.melt, aes(x = tipo, y = value,fill=variable)) +
  ...
  geom_text(data = dist.melt[dist.melt$value > 900,], aes(label =  format(value, big.mark="."), group=variable),
